...
import type { UpsertKafkaSinkConfig } from "@/composables/flink-jobs"
...
const rules = {
  ...
  keyFormat: [{required: true}],
  valueFormat: [{required: true}],
}

const {value, name} = defineProps<{
  value: UpsertKafkaSinkConfig,
  name: string,
}>()
      ...
      <FlinkKafkaTableFormItems
        ...
        <template #definedOptions>
          <a-form-item name="keyFormat" label="keyFormat" class="form-item-360">
            <a-input v-model:value="value.keyFormat" />
          </a-form-item>
          <a-form-item name="valueFormat" label="valueFormat" class="form-item-360">
            <a-input v-model:value="value.valueFormat" />
          </a-form-item>
          <a-form-item name="valueFieldsInclude" label="键列策略" class="form-item-360">
            <a-input v-model:value="value.valueFieldsInclude" />
          </a-form-item>
        </template>
      </FlinkKafkaTableFormItems>
      <a-flex-br />
