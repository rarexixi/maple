...
<template>
  <a-form ref="formRef" :model="detail" @finish="save" :rules="rules"
    ...
    <a-form-item ref="name" label="类型名称" name="name">
      <a-radio-group v-model:value="detail.name" type="text">
        <a-radio-button value="spark">Spark</a-radio-button>
        <a-radio-button value="flink">Flink</a-radio-button>
      </a-radio-group>
    </a-form-item>
    ...
    <a-form-item ref="engineHome" label="引擎目录" name="engineHome">
      ...
      <template v-if="detail.name == 'spark'">
        <SparkConfig v-model="detail.extInfo"/>
      </template>
      <template v-else-if="detail.name == 'flink'">
        <FlinkConfig v-model="detail.extInfo"/>
      </template>
