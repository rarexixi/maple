  ...
  <a-row>
    <a-col :span="12" style="height: calc(100vh - 42px); padding: 5px; overflow: auto;">
      ...
      <template v-for="(item, index) in mapleConfig.sources" :key="index">
        <add-plugin plugin-type="source" @add="(name: string) => addPlugin('source', name, index)" />
      ...
      <add-plugin plugin-type="transformation" @add="(name: string) => addPlugin('transformation', name)" />

      <a-divider />

      <a-typography-title :level="4">输出</a-typography-title>
      <template v-for="(item, index) in mapleConfig.sinks" :key="index">
        <add-plugin plugin-type="sink" @add="name => addPlugin('sink', name, index)" />
        <a-card :extra="item.name">
          <template #title>
      ...
      <add-plugin plugin-type="sink" @add="(name: string) => addPlugin('sink', name)" />
    </a-col>
    <a-col :span="12">
