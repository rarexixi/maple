...
val ${prefix}Path = <#if (config.path?matches(".*\\$\\{.*?}.*", "s"))>VariableUtils.replaceVariables("${path}", globalJavaVariables)<#else>"${path}"</#if>
...
<#list keys as key>
  ...
  .load(VariableUtils.replaceVariables(${prefix}Path, globalJavaVariables))
