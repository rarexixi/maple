...
${prefix}Query = VariableUtils.replaceVariables(${prefix}Query, globalJavaVariables)
<#else>
...
val ${prefix}Location = HiveSinkUtils.getLocation(spark, ${prefix}TargetTable, ${prefix}PartitionsColumns, globalVariables.toMap)
...
val ${prefix}Partition = ${prefix}PartitionsColumns.map(colName => s"$colName='${r"$"}{globalVariables.get(colName)}'").mkString(",")
