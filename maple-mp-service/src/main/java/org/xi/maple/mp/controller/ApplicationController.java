...
public class ApplicationController {
    ...
    @DeleteMapping("/{appNameList}")
    public ResponseEntity<Integer> deleteByAppName(
            @PathVariable("appNameList") @Validated List<@NotBlank(message = "appName(应用名称)不能为空") String> appNameList,
            @SetFieldTypes(types = {UPDATE}) BaseEntity baseEntity
    ) {
        Integer count = applicationService.deleteByAppName(appNameList, baseEntity);
        return ResponseEntity.ok(count);
    }

    @PatchMapping("/disable/{appNameList}")
    public ResponseEntity<Integer> disableByAppName(
            @PathVariable("appNameList") @Validated List<@NotBlank(message = "appName(应用名称)不能为空") String> appNameList,
            @SetFieldTypes(types = {UPDATE}) BaseEntity baseEntity
    ) {
        Integer count = applicationService.disableByAppName(appNameList, baseEntity);
        return ResponseEntity.ok(count);
    }

    @PatchMapping("/enable/{appNameList}")
    public ResponseEntity<Integer> enableByAppName(
            @PathVariable("appNameList") @Validated List<@NotBlank(message = "appName(应用名称)不能为空") String> appNameList,
            @SetFieldTypes(types = {UPDATE}) BaseEntity baseEntity
    ) {
        Integer count = applicationService.enableByAppName(appNameList, baseEntity);
        return ResponseEntity.ok(count);
    }
