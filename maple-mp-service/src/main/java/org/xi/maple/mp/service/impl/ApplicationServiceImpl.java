...
import org.xi.maple.mp.persistence.mapper.ApplicationMapper;
...
import org.xi.maple.mp.service.ApplicationService;
...
public class ApplicationServiceImpl implements ApplicationService {
    ...
    @Override
    @Transactional
    public int deleteByAppName(List<String> appNameList, BaseEntity baseEntity) {
        ApplicationPkCondition condition = getPkCondition(appNameList);
    ...
    @Override
    @Transactional
    public int disableByAppName(List<String> appNameList, BaseEntity baseEntity) {
        ApplicationPkCondition condition = getPkCondition(appNameList);
    ...
    @Override
    @Transactional
    public int enableByAppName(List<String> appNameList, BaseEntity baseEntity) {
        ApplicationPkCondition condition = getPkCondition(appNameList);
        ApplicationEntity entity = ObjectUtils.copy(baseEntity, ApplicationEntity.class);
    ...
    private ApplicationPkCondition getPkCondition(List<String> appNameList) {
        ApplicationPkCondition condition = new ApplicationPkCondition();
        if (appNameList.isEmpty()) {
            return null;
        } else if (appNameList.size() == 1) {
            condition.setAppName(appNameList.get(0));
        } else {
            condition.setAppNameIn(appNameList);
        }
        return condition;
    }
}
